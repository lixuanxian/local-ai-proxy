...
if (providerCount.c === 0) {
  ...
  // CLI providers — only seed if the binary is available
  const cliProviders = [
    ["claude-cli", "Claude CLI", "claude-cli", "claude", null, "https://api.anthropic.com", null, "claude-sonnet-4-6", JSON.stringify(["claude", "sonnet", "opus", "haiku"])],
    ["gemini-cli", "Gemini CLI", "gemini-cli", "gemini", "gemini", null, null, "gemini-3-flash-preview", JSON.stringify(["gemini", "gemma"])],
    ["copilot-cli", "GitHub Copilot", "copilot-cli", "copilot", "copilot", null, null, "claude-sonnet-4.6", null],
    ["codex-cli", "Codex CLI", "codex-cli", "codex", "codex", null, null, null, JSON.stringify(["codex"])],
  ];

  let defaultSet = false;
  for (const [id, name, type, cmd, command, baseUrl, apiKey, defaultModel, patterns] of cliProviders) {
    if (commandExists(cmd)) {
      const isDefault = !defaultSet ? 1 : 0;
      if (isDefault) defaultSet = true;
      insert.run(id, name, type, 1, isDefault, command, baseUrl, apiKey, defaultModel, patterns);
    }
  }

  // API providers — always seed (they connect to configurable endpoints)
  insert.run("ollama", "Ollama", "openai-api", 1, defaultSet ? 0 : 1, null, (process.env.OLLAMA_HOST || "http://localhost:11434") + "/v1", null, "llama3",
