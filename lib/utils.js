 ...
 * Handles multimodal content arrays gracefully.
 */
...
function resolveProvider(providerConfig, providerRegistry) {
  ...
  if (providerConfig.type === "openai-api" && providerConfig.base_url) {
    ...
    return new BaseAPIProvider({
      ...
      buildBody(messages, model, stream) {
        ...
        return { model: effectiveModel, messages, stream };
      },
  ...
  const typeMap = {
    'cli': 'claude', 'copilot-cli': 'copilot', 'codex-cli': 'codex',
    'aider-cli': 'aider', 'opencode-cli': 'opencode', 'gemini-cli': 'gemini',
    'gemini-api': 'gemini',
  };
...
module.exports = {
  formatMessages, makeResponse, resolveProvider, extractText,
