  ...
  resolve(providerName, model) {
    ...
    if (model) {
      ...
      if (mapping && mapping.provider_id) {
        ...
        if (this._providers.has(registryName)) return { name: registryName, modelMatched: true };
      }
    }
    ...
    if (defaultDb && defaultDb.enabled) {
      ...
      if (this._providers.has(defaultName)) return { name: defaultName, modelMatched: false };
    }
    ...
    for (const p of allProviders) {
      if (!p.enabled) continue;
      const registryName = dbToRegistryName(p);
      if (this._providers.has(registryName)) return { name: registryName, modelMatched: false };
    }
...
function createDefaultRegistry() {
  ...
  const builtins = {
    'claude-cli': "../providers/claude-cli",
