...
const stmtGetApiTokenByToken = db.prepare("SELECT * FROM api_tokens WHERE token = ? AND enabled = 1");
...
function saveApiToken(tok) {
  ...
  db.prepare(`
    INSERT OR REPLACE INTO api_tokens (id, name, token, enabled, created_at, last_used_at)
    VALUES (?, ?, ?, ?, COALESCE((SELECT created_at FROM api_tokens WHERE id = ?), datetime('now')), (SELECT last_used_at FROM api_tokens WHERE id = ?))
  `).run(id, tok.name, tok.token, tok.enabled !== undefined ? (tok.enabled ? 1 : 0) : 1, id, id);
  return id;
}
