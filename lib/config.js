...
function saveConversation(conv) {
  ...
  db.prepare(`
    INSERT OR REPLACE INTO conversations (id, title, provider_id, model, system_prompt, temperature, max_tokens, created_at, updated_at)
  ...
  `).run(id, conv.title || "New Chat", conv.provider_id || null, conv.model || null,
    conv.system_prompt || null, conv.temperature ?? null, conv.max_tokens ?? null, id);
  return id;
}
...
function saveMessage(msg) {
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, datetime('now'))
  `).run(id, msg.conversation_id, msg.role, msg.content || "", msg.attachments ? JSON.stringify(msg.attachments) : null);
...
module.exports = {
  ...
  getAllProviders, getProvider, saveProvider, deleteProvider, setDefaultProvider, getDefaultProvider, bulkToggleProviders,
  ...
  getMessages, saveMessage, updateMessageContent, deleteMessage,
