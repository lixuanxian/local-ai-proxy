...
function saveConversation(conv) {
  ...
  db.prepare(`
    INSERT OR REPLACE INTO conversations (id, title, provider_id, model, created_at, updated_at)
    VALUES (?, ?, ?, ?, COALESCE((SELECT created_at FROM conversations WHERE id = ?), datetime('now')), datetime('now'))
  `).run(id, conv.title || "New Chat", conv.provider_id || null, conv.model || null, id);
  return id;
}
