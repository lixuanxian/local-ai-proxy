 ...
 */
function queryLogs({ page = 1, limit = 50, provider, model, search, since, until, status } = {}) {
  let where = "1=1";
  const params = [];
  ...
  if (search) {
    where += " AND (model LIKE ? OR provider_id LIKE ? OR api_format LIKE ?)";
    params.push(`%${search}%`, `%${search}%`, `%${search}%`);
  }
  if (status === 'error') {
    where += " AND (error IS NOT NULL OR status_code >= 400)";
  } else if (status === 'success') {
    where += " AND error IS NULL AND (status_code IS NULL OR status_code < 400)";
  }
  if (since) {
    where += " AND timestamp >= ?";
    params.push(since);
...
function getStats() {
  ...
  const tokens = db.prepare("SELECT COALESCE(SUM(input_tokens), 0) as input, COALESCE(SUM(output_tokens), 0) as output FROM request_logs").get();

  return { total, today, avgLatency: Math.round(avgLatency), errors, byProvider, totalInputTokens: tokens.input, totalOutputTokens: tokens.output };
}
...
module.exports = { logRequest, loggerMiddleware, queryLogs, getStats, getHourlyStats, getProviderStats, getModelStats, clearLogs };
