 ...
 * Uses @modelcontextprotocol/sdk with dynamic import() for ESM compatibility.
 */
...
async function getSDK() {
  if (!_sdk) {
    _sdk = await import("@modelcontextprotocol/sdk/client/index.js");
  }
  return _sdk;
}
...
async function getSSETransport() {
  if (!_sseTransport) {
    _sseTransport = await import("@modelcontextprotocol/sdk/client/sse.js");
  }
...
async function getStreamableTransport() {
  if (!_streamableTransport) {
    _streamableTransport = await import("@modelcontextprotocol/sdk/client/streamableHttp.js");
  }
...
class McpClientManager {
  ...
  async _createTransport(serverConfig) {
    ...
    if (serverConfig.transport_type === "sse") {
      const { SSEClientTransport } = await getSSETransport();
    ...
    const { StreamableHTTPClientTransport } = await getStreamableTransport();
