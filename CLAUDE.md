...
Local AI Proxy is a unified API gateway that routes chat requests to multiple AI providers through both OpenAI-compatible and Anthropic-compatible endpoints. It includes a React web dashboard for management, SQLite persistence, built-in chat with MCP tool integration, and standalone executable packaging.
...
lib/
  ...
  paths.js             # Path resolution — real filesystem paths in pkg mode, project-relative in dev
  runtime-extract.js   # Extracts embedded assets from pkg snapshot on first run
providers/
...
web/                   # React frontend (Vite + Ant Design)
  src/
    ...
    components/        # ErrorBoundary, CommandPalette, Skeleton
    pages/             # Dashboard, Chat, Providers, Logs, Apps, Settings, NotFound
scripts/               # Build & packaging scripts
  config.js            # Shared platform configuration (single source of truth)
  dist.js              # Unified build pipeline orchestrator
  build-server.js      # esbuild bundler (server.js → dist/server.bundle.cjs)
  build-icon.js        # SVG → ICO converter (Windows only)
  post-pkg.js          # Copies companion files next to exe after pkg
  set-icon.js          # Embeds icon + version info into Windows exe (resedit)
  pack-dist.js         # Verifies companion files + creates zip archive
public/                # Built frontend assets (generated by `cd web && npm run build`)
...
- **Build config centralized** — all platform targets/names in `scripts/config.js`, shared by `dist.js`, `pack-dist.js`, and CI workflow
